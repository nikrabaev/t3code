...
- **`WeaveDecisionSurfacer`** — on `weave.node.verified`, recomputes look-ahead; if any open PendingDecision is within _N_ ready-edges of a Node whose readiness depends on it, emits `weave.decision.surfaced`. v0.1 look-ahead window: 1 (only when a Node is literally next). v0.2: tune.
...
> _"Superpowers is loaded. USE: `test-driven-development`, `verification-before-completion`, `systematic-debugging`, `requesting-code-review`. SUPPRESS: `brainstorming`, `writing-plans`, `finishing-a-development-branch`, `subagent-driven-development` — these are handled by Weave at the project layer. You are one Node in a larger plan; do not attempt to re-plan. Your scope is limited to the files listed in writeSet below."_
